...
- **Market ingestion**: Uses the Gamma API first (with server-side filters `active=true`, `closed=false`, `archived=false`), caches results for 30 seconds, and only falls back to a limited CLOB pagination sweep if Gamma produces zero tradable markets. Missing `acceptingOrders`/`enableOrderBook` flags default to opt-in so “soft” states do not erase the feed.
