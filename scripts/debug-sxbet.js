...
const WALLET_ADDRESS = process.env.SXBET_WALLET_ADDRESS;
const PRIVATE_KEY = process.env.SXBET_PRIVATE_KEY;
const BASE_TOKEN = '0x6629Ce1Cf35Cc1329ebB4F63202F3f197b3F050B';

// Check environment variables
console.log('🔧 Environment Variable Check:');
console.log(`   SXBET_API_KEY: ${API_KEY ? '✅ Set (length: ' + API_KEY.length + ')' : '❌ Not set'}`);
console.log(`   SXBET_WALLET_ADDRESS: ${WALLET_ADDRESS ? '✅ Set' : '❌ Not set'}`);
console.log(`   SXBET_PRIVATE_KEY: ${PRIVATE_KEY ? '✅ Set' : '❌ Not set'}`);
console.log('');

// If no API key, show instructions
if (!API_KEY) {
  console.log('❌ SXBET_API_KEY is required for SX.bet API access');
  ...
  console.log('   3. Set SXBET_API_KEY in your environment variables');
  console.log('');
  process.exit(1);
}
